...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto">
        <h1 class="text-3xl font-bold text-gray-800 mb-8">Edit Hotel</h1>

        <form action="{{ route('hotels.update', $hotel) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $hotel->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="mt-2 grid grid-cols-2 md:grid-cols-3 gap-4">
                        @if($hotel->images && count($hotel->images) > 0)
                            @foreach($hotel->images as $index => $image)
                                <div class="relative">
                                    <img src="{{ Storage::url($image) }}" alt="Hotel Image {{ $index + 1 }}" class="w-full h-32 object-cover rounded">
            ...
            <div class="mt-8 flex justify-end">
                <a href="{{ route('hotels.show', $hotel) }}" class="mr-4 bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded">
                    Cancel
                </a>
